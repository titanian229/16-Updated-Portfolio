...
var renderProject = function renderProject(project) {
  var template = fs.readFileSync(path.resolve(__dirname, "project-template.html"), "utf8");
  ...
  technologies = project.technologies.map(function (techItem) {
    return "<li>".concat(techItem, "</li>");
  });
  template = replacePlaceholders(template, "ProjectTechnologies", technologies.join("\n"));

  if (project.githubLink != "") {
    githubButtonTemplate = "<a href=\"".concat(project.githubLink, "\"><i class=\"fab fa-github\"></i></a>");
...
var replacePlaceholders = function replacePlaceholders(template, placeholder, value) {
  var pattern = new RegExp("{{ " + placeholder + " }}", "gm");
  return template.replace(pattern, value);
};

var projectSection = projects.map(function (project) {
